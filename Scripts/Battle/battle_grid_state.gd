...
func is_spell_target_valid(caster : Unit, coord : Vector2i, spell : BattleSpell) -> bool:
	match spell.name:
		...
		"Fireball": # any hex target is valid
			return true
		"Teleport", "Wind Dash": # empty tile in range that is in front of the caster
		...
		"Blood Ritual":  # any enemy units, when enemy army has at least 3 units
			var target : Unit = get_unit(coord)
			if target and target.controller.team != caster.controller.team:  # enemy unit
				if target.army_in_battle.units.size() >= 3:  # target army has at least 3 alive units
					return true
		_:
