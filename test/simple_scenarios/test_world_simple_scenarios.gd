...
func test_world_map_start_and_close() -> void:
	gut.p("click 'Full game' in lobby")
	var full_game_button = get_node(test_UI_PATHS.WORLD_MODE_BUTTON_PATH)
	...
	var world_ui = get_node(test_UI_PATHS.WORLD_UI_PATH)
